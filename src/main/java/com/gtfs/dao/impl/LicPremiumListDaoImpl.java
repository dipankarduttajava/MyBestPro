...
public class LicPremiumListDaoImpl implements LicPremiumListDao,Serializable{
	...
	public List<Long> findPremiumListForRenewalPicDelivery(List<LicHubMst> licHubMsts){
        ...
        try {
            ...
            criteria.add(Restrictions.ne("lpd.payMode","C"));
	...
	@Override
	public List<LicOblApplicationMst> findPremiumListReport(List<LicHubMst> findHubForProcess, Date businessFromDate, Date businessToDate, BranchMst branchMst) {
		List<LicOblApplicationMst> list = null;
	 	Session session = null;
        try {
            session = sessionFactory.openSession();
            ...
            Query query = session.createQuery("SELECT loam FROM LicOblApplicationMst as loam inner join "
                    ...
                    + "loam.branchMst as bm inner join fetch "
                    + "loam.oblHubMst as ohm inner join fetch "
                    + "lpvm.licProductMst as lipm inner join fetch "                    
                    + "loam.licPremApplMappings as lpam inner join "
                    + "loam.licPisMst as lpms left outer join "
                    + "loam.licMarkingToQcDtls as lmtqd "
                    
					+ "WHERE "
					+ "loam.businessDate >= :businessFromDate "
					+ "AND loam.businessDate <= :businessToDate "
					//+ "AND loam.oblHubMst IN :findHubForProcess "
           ...
           // query.setParameter("findHubForProcess", findHubForProcess);
